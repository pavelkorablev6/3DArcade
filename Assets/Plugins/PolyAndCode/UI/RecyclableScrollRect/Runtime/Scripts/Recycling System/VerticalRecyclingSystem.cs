...
namespace PolyAndCode.UI
{
    ...
    public class VerticalRecyclingSystem : RecyclingSystem
    {
        ...
        private void CreateCellPool()
        {
            ...
            if (_isGrid)
            {
                SetTopLeftAnchor(_prototypeCell);
            }
            else
            {
                SetTopAnchor(_prototypeCell);
            }
            ...
            _cellWidth = _content.rect.width / _coloumns;
            _cellHeight = _prototypeCell.sizeDelta.y / _prototypeCell.sizeDelta.x * _cellWidth;

            //Get the required pool coverage and mininum size for the Cell pool
            float requriedCoverage = _minPoolCoverage * _viewport.rect.height;
        ...
        private Vector2 RecycleTopToBottom()
        {
            ...
            int n = 0;
            float posY = _isGrid ? _cellPool[_bottomMostCellIndex].anchoredPosition.y : 0;
            float posX = 0;
            ...
            while (_cellPool[_topMostCellIndex].MinY() > _recyclableViewBounds.max.y && _currentItemCount < DataSource.GetItemCount())
            {
                if (_isGrid)
                {
                    if (++_bottomMostCellColoumn >= _coloumns)
            ...
            if (_isGrid)
            {
                _content.sizeDelta += _cellHeight * additionalRows * Vector2.up;
            ...
            _cellPool.ForEach((RectTransform cell) => cell.anchoredPosition += _cellPool[_topMostCellIndex].sizeDelta.y * n * Vector2.up);
            _content.anchoredPosition -= _cellPool[_topMostCellIndex].sizeDelta.y * n * Vector2.up;
            _recycling = false;
        ...
        private Vector2 RecycleBottomToTop()
        {
            ...
            int n = 0;
            float posY = _isGrid ? _cellPool[_topMostCellIndex].anchoredPosition.y : 0;
            float posX = 0;

            //to determine if content size needs to be updated
            int additionalRows = 0;
            //Recycle until cell at bottom is avaiable and current item count is greater than cellpool size
            while (_cellPool[_bottomMostCellIndex].MaxY() < _recyclableViewBounds.min.y && _currentItemCount > _cellPool.Count)
            {
